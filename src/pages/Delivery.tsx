...
const Delivery: React.FC = () => {
  ...
  const { data, error, isLoading } = useSWR(
    `${import.meta.env.VITE_BACKEND_URL}/api/static-pages/delivery`,
    (url) => axios.get(url).then((res) => res.data.data)
  );

  if (isLoading) {
    return <LoadingState message="Loading delivery information..." />;
  }

  if (error) {
    return <ErrorState message="Failed to load delivery information" />;
  }
